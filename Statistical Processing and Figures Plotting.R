...
#Calculate average Normalized_Int_vol for each brain region in the dataframe for obese samples
obese_samples_df$Average_Normalized_Int_vol <- rowMeans(obese_samples_df[, grep("Normalized_Int_vol", names(obese_samples_df))], na.rm = TRUE)

#Create a summary dataframe containing only the RegionAbbr and Average_Normalized_Int_vol columns for obese samples
obese_summary_df <- obese_samples_df[, c("RegionAbbr", "Average_Normalized_Int_vol")]

#Calculate average Normalized_Int_vol for each brain region in the dataframe for lean samples
lean_samples_df$Average_Normalized_Int_vol <- rowMeans(lean_samples_df[, grep("Normalized_Int_vol", names(lean_samples_df))], na.rm = TRUE)

#Create a summary dataframe containing only the RegionAbbr and Average_Normalized_Int_vol columns for lean samples
lean_summary_df <- lean_samples_df[, c("RegionAbbr", "Average_Normalized_Int_vol")]
...
obese_samples_df <- obese_samples_df[significant_criteria,c("RegionAbbr", "sample4974_Normalized_Int_vol", "sample4985_Normalized_Int_vol", "sample4986_Normalized_Int_vol", "sample7073_Normalized_Int_vol",
                                       "sample9294_Normalized_Int_vol", "sample9390_Normalized_Int_vol", "sample9272_Normalized_Int_vol", "sample9269_Normalized_Int_vol")]
lean_samples_df <- lean_samples_df[significant_criteria, c("RegionAbbr","sample4932_Normalized_Int_vol", "sample4933_Normalized_Int_vol","sample8293_Normalized_Int_vol",
                                        "sample8294_Normalized_Int_vol", "sample8405_Normalized_Int_vol", "sample8406_Normalized_Int_vol")]
...
generate_and_save_plots(parent_label)

##Plot heatmap
#Filter only significant brain regions
obese_summary_df <- obese_summary_df[significant_criteria,]
lean_summary_df <- lean_summary_df[significant_criteria,]

#Add a 'Group' column to indicate the group (Obese/Lean)
obese_summary_df$Group <- "Obese"
lean_summary_df$Group <- "Lean"

#Combine both data frames into one
combined_df <- rbind(obese_summary_df, lean_summary_df)
...
heatmap_data <- matrix(combined_df$Average_Normalized_Int_vol, nrow = nrow(combined_df)/2, ncol = 2, byrow = FALSE)

#Create a new column that combines parent labels and brain region abbreviations
combined_labels <- paste(merged_df[, 2], combined_df$RegionAbbr[1:nrow(heatmap_data)], sep = " ")
...
colnames(heatmap_data) <- c("Obese", "Lean")

#Swap columns to standardize with boxplot arrangement of lean samples followed by obese
heatmap_data <- heatmap_data[, c("Lean", "Obese")]
...
grid.text("Groups", x = 0.37, y = -0.02, gp = gpar(fontsize = 22))
...
Number_of_Significant_Regions <- rep(1, length(parent_label))

#Create dataframe to store number of significant regions in parental brain regions
Count_significant_regions <- data.frame(parent_label,Number_of_Significant_Regions)
...
#Calculate percentage of significant regions
Count$Percentage <- Count$Number_of_Significant_Regions / total_count * 100
...
ggplot(Count, aes(x = "", y = Percentage, fill = parent_label)) +
  geom_bar(width = 1, stat = "identity") +
  geom_text(aes(label = paste0(Number_of_Significant_Regions, "/", total_count)),
            position = position_stack(vjust = 0.5), size = 5) +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = setNames(custom_palette, unique(parent_label))) +  # Apply custom color palette
...
PCA_combined <- PCA_combined[,significant_criteria]
...
significant_regions <- cbind(parent_label,significant_regions)
